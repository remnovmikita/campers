...
function cleanFilters(filters: FormValues) {
...
export const getAllCampers = async (page: number, filters: FormValues) => {
  const res = await axios.get<AllCampersResponse>(
...
export const GetOneCamper = async (campersId: string) => {
  const data = axios.get(
    `${process.env.NEXT_BACKEND_API}/campers/${campersId}`,
  );
  return data;
};
...
export const getFilterCampers = async () => {
  const res = await axios.get<Filters>(
    `https://campers-api.goit.study/campers/filters`,
  );
  return res.data;
};
...
export const getReviesbyId = async (campersId: string) => {
  const res = await axios.get<DataReviews[]>(
    `${process.env.NEXT_BACKEND_API}/campers/${campersId}/reviews`,
  );
  return res.data;
};
...
export const postReqest = async (campersId: string, value: FormValuesPost) => {
  const res = await axios.post(
    `https://campers-api.goit.study/campers/${campersId}/booking-requests`,
    value,
  );
  return res.data;
};
